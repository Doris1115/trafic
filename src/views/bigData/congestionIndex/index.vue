  ...
  <div class="app-container">
    ...
    <div class="head-container">
      <div>
        ...
        <el-input v-model="roadName" clearable placeholder="道路名称" class="filter-item" style="width:200px" @keyup.enter.native="queryData" />
        <!-- 搜索 -->
        <el-button class="filter-item" size="mini" type="success" icon="el-icon-search" @click="queryData">搜索</el-button>
      </div>
      <el-dialog :close-on-click-modal="false" :before-close="dialogClose" :visible.sync="chartShow" custom-class="chartModel" :title="trendTitle">
        <el-row class="line-box">
          <line-chart :chart-data="trendData" />
        </el-row>
      </el-dialog>
      <!--表格渲染-->
      <el-table ref="table" v-loading="tableLoading" :data="tableData" size="mini" style="width: 100%;" height="800">
        ...
        <el-table-column v-for="(item,index) in tableColumnProp" :key="index" :label="item.title" width="80">
          <el-table-column :prop="item.indexAvg" label="今日" width="80" :formatter="numberFormatter" />
          <el-table-column :prop="item.lastMonthThisDayIndexAvg" label="上月同期" width="80" :formatter="numberFormatter" />
          <el-table-column :prop="item.lastYearThisDayIndexAvg" label="去年同期" width="80" :formatter="numberFormatter" />
          <el-table-column :prop="item.yesterdayIndexAvg" label="昨日" width="80" :formatter="numberFormatter" />
        </el-table-column>
      </el-table>
      <!--分页组件-->
      <!-- <pagination /> -->
