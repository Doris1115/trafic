  ...
  methods: {
    ...
    queryData() {
      ...
      congestionIndex(param).then(res => {
        this.tableData = res.tableData.map((item, index) => {
          var am = {}
          Object.keys(item).filter(el => {
            return !el.indexOf('am')
          }).forEach(el => {
            am[el] = {
              indexAvg: parseFloat(item[el].indexAvg).toFixed(2),
              yesterdayIndexAvg: parseFloat(item[el].yesterdayIndexAvg).toFixed(2),
              lastMonthThisDayIndexAvg: parseFloat(item[el].lastMonthThisDayIndexAvg).toFixed(2),
              lastYearThisDayIndexAvg: parseFloat(item[el].lastYearThisDayIndexAvg).toFixed(2)
            }
          })
          return {
            ...am,
