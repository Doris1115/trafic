...
// import '../base/mapapi/map_load.js'
// import '../base/mapapi/bmap_offline_api_v3.0_min.js'
...
export default {
  ...
  methods: {
    ...
    getMapScript() {
      if (!global.BMap) {
        console.log("百度地图竟然加载失败")
        const ak = this.ak || this._BMap().ak
        global.BMap = {}
        global.BMap._preloader = new Promise((resolve, reject) => {
          global._initBaiduMap = function() {
            resolve(global.BMap)
            // global.document.body.removeChild($scriptConfig)
            global.document.body.removeChild($scriptApi)
            global.BMap._preloader = null
            global._initBaiduMap = null
          }
          const $scriptConfig = document.createElement('script')
          global.document.body.appendChild($scriptConfig)
          $scriptConfig.src = `/static/js/map_load.js`
          const $scriptApi = document.createElement('script')
          global.document.body.appendChild($scriptApi)
          // $script.src = `https://api.map.baidu.com/api?v=2.0&ak=${ak}&callback=_initBaiduMap`
          $scriptApi.src = `/static/js/bmap_offline_api_v3.0_min.js`
        })
