    ...
    <template v-if="mode === 'transfer'">
      ...
      <div class="transfer-left">
        ...
        <div class="transfer-main">
          ...
          <el-tree ref="from-tree" show-checkbox :lazy="lazy" :node-key="node_key" :load="leftloadNode"
            :props="defaultProps" :data="self_from_data" :default-expand-all="openAll" :highlight-current="highLight"
      ...
      <div class="transfer-right">
        ...
        <div class="transfer-main">
          ...
          <el-tree slot="to" ref="to-tree" show-checkbox :lazy="lazyRight" :data="self_to_data" :node-key="node_key"
            :props="defaultProps" :load="rightloadNode" :default-expand-all="openAll" :highlight-current="highLight"
            :render-content="renderContentRight" :filter-node-method="filterNodeTo"
            :default-expanded-keys="to_expanded_keys" @check="toTreeChecked"></el-tree>
    ...
    <template v-else-if="mode === 'addressList'">
      ...
      <div class="transfer-left">
        ...
        <div class="transfer-main">
          ...
          <el-tree ref="from-tree" show-checkbox :node-key="node_key" :props="defaultProps" :data="self_from_data"
            :default-expand-all="openAll" :highlight-current="highLight" :render-content="renderContentLeft"
            :filter-node-method="filterNodeFrom" :default-expanded-keys="from_expanded_keys" @check="fromTreeChecked">
          </el-tree>
        </div>
      </div>
      <!-- 穿梭区 按钮框 -->
      <div class="transfer-center address-list-center">
        <p class="transfer-center-item" v-show="!move_up" :class="{ 'address-only-item': addressOptions.num === 1 }">
          <el-button type="primary" @click="addressListTransfer(0)" icon="el-icon-arrow-right" circle
            class="address-first-btn" :disabled="from_disabled"></el-button>
        </p>
        <p class="transfer-center-item" v-if="addressOptions.num > 1">
          <el-button type="primary" @click="addressListTransfer(1)" :disabled="from_disabled" icon="el-icon-arrow-right"
            circle></el-button>
        </p>
        <p class="transfer-center-item" v-show="move_up">
          <el-button type="primary" @click="addressListTransfer(2)" :disabled="from_disabled" icon="el-icon-arrow-right"
            circle></el-button>
        </p>
      </div>
      <div class="transfer-right">
        <div class="transfer-right-item" :class="{
          ...
          <div class="transfer-main" v-if="!move_up">
            <slot name="to"></slot>
            <el-input v-if="filter" v-model="filterListFirst" :placeholder="placeholder" size="small"
        ...
        <div class="transfer-right-item" v-if="addressOptions.num >= 2">
          ...
          <div class="transfer-main">
            <slot name="to"></slot>
            <el-input v-if="filter" :placeholder="placeholder" v-model="filterListSecond" size="small"
              class="filter-tree"></el-input>
            <ul class="address-list-ul">
        ...
        <div v-if="addressOptions.num === 3" class="transfer-right-item" :class="{ 'transfer-right-small': !move_up }">
          <h3 class="transfer-title">
          ...
          <div class="transfer-main" v-if="move_up">
            <slot name="to"></slot>
            <el-input v-if="filter" :placeholder="placeholder" v-model="filterListThird" size="small"
              class="filter-tree"></el-input>
            <ul class="address-list-ul">
...
export default {
  ...
  computed: {
    // 左侧数据
    self_from_data() {
      const from_array = [...this.from_data]
      if (!this.arrayToTree) {
  ...
  watch: {
    ...
    defaultExpandedKeys(val) {
      const _form = new Set(this.from_expanded_keys.concat(val))
      this.from_expanded_keys = [..._form]
      const _to = new Set(this.to_expanded_keys.concat(val))
  ...
  methods: {
    ...
    addToAims(emit) {
      ...
      newSkeletonHalfCheckedNodes.forEach(item => {
        ...
        // this.$refs['to-tree'].setChecked(item, true)
      })
      ...
      // this.to_disabled=false
      // leafCheckedNodes.forEach(item => {
      //   this.toTreeChecked(item, leafCheckedNodes)
      // })

      // this.to_check_keys = leafCheckedNodes
      // 递归查询data内是否存在item函数
    ...
    removeToSource() {
      ...
      // item[pid__][children__] && item[pid__][children__].length > 0? this.$refs['from-tree'].insertBefore(item, item[pid__][children__][0]) : this.$refs['from-tree'].append(item, item[pid__])
      // 第二步
      // 筛选目标树不存在的骨架节点 - 全选内的节点
      let newSkeletonCheckedNodes = []
      nodes.forEach(item => {
