...
function CRUD(options) {
  ...
  const data = {
    ...
    status: {
      ...
      check: CRUD.STATUS.NORMAL,
      // 添加或编辑状态
      get cu() {
        if (this.add === CRUD.STATUS.NORMAL && this.edit === CRUD.STATUS.NORMAL && this.check === CRUD.STATUS.NORMAL) {
          return CRUD.STATUS.NORMAL
        } else if (this.add === CRUD.STATUS.PREPARED || this.edit === CRUD.STATUS.PREPARED || this.check === CRUD.STATUS.PREPARED) {
          return CRUD.STATUS.PREPARED
        } else if (this.add === CRUD.STATUS.PROCESSING || this.edit === CRUD.STATUS.PROCESSING || this.check === CRUD.STATUS.PROCESSING) {
      ...
      // 标题
      get title() {
        return this.check > CRUD.STATUS.NORMAL? `查看${crud.title}` :this.add > CRUD.STATUS.NORMAL ? `新增${crud.title}` : this.edit > CRUD.STATUS.NORMAL ? `编辑${crud.title}` : crud.title
      }
    },
    msg: {
      submit: '提交成功',
  ...
  const methods = {
    ...
    cancelCU() {
      ...
      const checkStatus = crud.status.check
      ...
      if (checkStatus === CRUD.STATUS.PREPARED) {
        // if (!callVmHook(crud, CRUD.HOOK.beforeEditCancel, crud.form)) {
        //   return
        // }
        crud.status.check = CRUD.STATUS.NORMAL
        crud.getDataStatus(crud.getDataId(crud.form)).check = CRUD.STATUS.NORMAL
      }
      crud.resetForm()
      if (addStatus === CRUD.STATUS.PREPARED) {
