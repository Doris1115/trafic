window.BMAP_AUTHENTIC_KEY = "199471448";
var mapapi=
(function() {
	var b2, a8 = b2 = a8 || {
		version: "2.0"
	...
	a8.dom.getPosition = function(T) {
		...
		var cT = a8.dom.getDocument(T),
			cN = a8.browser,
			cQ = a8.dom.getStyle,
			cM = cN.isGecko > 0 && cT.getBoxObjectFor && cQ(T, "position") == "absolute" && (T.style.top === "" || T.style.left === ""),
			cR = {
				left: 0,
				top: 0
			},
		...
		if (T.getBoundingClientRect) {
			...
			if (cN.opera > 0 || (cN.isWebkit > 0 && cQ(T, "position") == "absolute")) {
	...
	window._addStat = function(cP, cO) {
		...
		var cQ = function(cR) {
			...
			setTimeout(function() {
				_addStat._img.src = cc.imgPath + "blank.gif";
			}, 50);
		};
		var T = function() {
				...
				if (T._status == this.Request.INITIAL) {
					...
					if (cM.delayFlag == false) {
						...
						window.setTimeout(function() {
							var cO = cM.Config._baseUrl + "modules.js";
							cx.request(cO);
	...
	aT.getTilesUrl = function(cM, cP) {
		var cQ = cM.x;
		var cN = cM.y;
        cL = mapserver + "maptile/roadmap/" + cP + "/" + cQ + "/" + cN + ".png";
        console.log(cl)
	...
	bs.getTilesUrl = function(T, cM) {
		...
		url = this.tileUrls[Math.abs(cO + cL) % this.tileUrls.length] + this.map.currentCity + "/" + this.map.cityCode + "/3/lv" + (21 - cM) + "/" + cO + "," + cL + ".png";
		return url;
	};
	...
	bQ.getTilesUrl = function(cL, cN) {
		var cO = cL.x;
		var cM = cL.y;
		var T = b7[Math.abs(cO + cM) % b7.length] + "u=x=" + cO + ";y=" + cM + ";z=" + cN + ";v=009;type=sate&fm=46";
		return mapserver + "maptile/midnight/" + cN + "/" + cO + "/" + cM + ".jpg";
		return T.replace(/-(\d+)/gi, "M$1");
	};
	...
	o.getTilesUrl = function(cM, cO) {
		...
		var cN = cM.y;
		var T = "20130617";
		return mapserver + "maptile/overlay/" + cO + "/" + cP + "/" + cN + ".png";
	...
	window.BMAP_HYBRID_MAP = new cu("\u5927\u5c4f", [bQ, o], {
		tips: "\u663e\u793a\u5e26\u6709\u8857\u9053\u7684\u536b\u661f\u5f71\u50cf",
		labelText: "\u8def\u7f51",
		minZoom: 1,
		maxZoom: 19,
